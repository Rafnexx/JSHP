...
function spareMoneyFeedback(arr, bankMoney, devicePrice) {
  let GOAL = devicePrice;
  let actualSum = arr.reduce(function(prev, next) {
    return prev + next;
  }, bankMoney);

  let result =
    actualSum >= GOAL
      ? `Masz ${actualSum} zł łącznie, mozesz sobie pozwolić na zakup sprzętu`
      : `Masz ${actualSum} zł łącznie, brakuje Ci ${GOAL -
          actualSum} zł. Przykro mi.`;
